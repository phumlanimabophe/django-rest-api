...
class BrowsableAPIRenderer(BaseRenderer):
    ...
    def get_context(self, data, accepted_media_type, renderer_context):
        ...
        excel_file = file_data.pop('excel_file', None)
        if excel_file is not None:
            from django.core.files.uploadedfile import InMemoryUploadedFile
        # Initialize excel_data_done to an empty string
        excel_data_done = ''

        # Check if the file is an InMemoryUploadedFile
        if excel_file is not None:
            if isinstance(excel_file[0], InMemoryUploadedFile):
                # If it is, read the file directly from the InMemoryUploadedFile
                excel_data = pd.read_excel(excel_file[0], engine='openpyxl')
                excel_data_done = excel_data.to_json(orient='records')
            else:
                # If it's not, read the file as before
                excel_data = pd.read_excel(BytesIO(excel_file[0].read()), engine='openpyxl')
                excel_data_done = excel_data.to_json(orient='records')

        return {
            'graph_data': excel_data_done,
            'added': excel_file is None,
            'content':  data,
